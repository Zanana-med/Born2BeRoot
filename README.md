...
+ **Lighttpd :** A lightweight fast webs server, its job to handle incoming requests from browsers and deliver the file they ask for, such HTML, CSS, images or dynamic page generated by PHP.  
  Imagine a visitor types your website's address. Lightpptd receive ths request, checks which file is needed and sends it back to the browser so the user can see the webpage.
  
+ **Maria-DB :** is a database management system that stores all the data for your website in a highly structured way using tables and rows.
